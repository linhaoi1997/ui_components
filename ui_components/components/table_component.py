...
from ..utils.base_actions import click
import re
...
class TValues(object):
    ...
    def select(self):
        if self.status:
            ...
            self.element.click()
            WebDriverWait(self.element.parent, 5).until(lambda x: self.status is True, "点击失败，未知原因")

    def send_keys(self, value):  # 列表可能是输入框
        self.element.find_element_by_xpath(".//input").send_keys(value)
...
class NoPageInfoTableComponent(BaseComponent):
    DEFAULT_LOCATOR = "//table"

    def _handle(self):
        super(NoPageInfoTableComponent, self)._handle()
...
class TableComponent(NoPageInfoTableComponent):
    page_finder = re.compile("(\d+)-(\d+)\s/\s(\d+)")

    def _handle(self):
        super(TableComponent, self)._handle()
